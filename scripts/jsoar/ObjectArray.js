...
export class ObjectArray {
    ...
    static #isConstructorLocked = true;   // private constructor lock
    constructor(data) {
        if (ObjectArray.#isConstructorLocked)
            HTMLOutput.showError("Cannot initialize using 'new'. Call createInstance() instead.");
        ObjectArray.#isConstructorLocked = true;

        this.#data = data;
        this.#validator = new ParameterValidator(this, structuredClone(data[0]));
    }
     ...
     * @param {JSON[]} jsonData
     * @returns instance of ObjectArray
     */
    static createInstance(jsonData) {
        // check if whole data is array
        if (!Array.isArray(jsonData))
            HTMLOutput.showError("Provided data is not a array.");
        ...
        if (!isObject)
            HTMLOutput.showError("Provided data is not array of objects.");

        // check objects' unformity
        const matchKeys = new Set(Object.keys(jsonData[0]));
        const isUnformity = jsonData.every(item => {
        ...
        if (!isUnformity)
            HTMLOutput.showError("Provided data array does not have uniform objects.");

        ObjectArray.#isConstructorLocked = false;
        return new ObjectArray(jsonData);
    }

    /**
     * simply to call ObjectArray's private constructor
     * @param {JSON[]} jsonData
     * @returns instance of ObjectArray
     */
    static #internalCreateInstance(jsonData) {
    ...
    // GETTERS
    /**
     * gets the valdiator for calling validation functions from outside
     */
    get validator() {
     ...
     */
    get data() {
        // creating issues when doing with proxies
